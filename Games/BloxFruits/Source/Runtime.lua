if _G.DragonSelectedModePreset then
    ...
    _G.DragonDetectedWorld = (PlaceId == 2753915549 or PlaceId == 85211729168715) and "Sea 1" or (PlaceId == 4442272183 or PlaceId == 79091703265657) and "Sea 2" or "Sea 3"
...
_G.DragonLoadFeatureModule = function(Name)
    if not Name or _G.DragonLoadedFeatureModules[Name] then
        return _G.DragonLoadedFeatureModules[Name]
    end
    local Link = _G.DragonLinks and _G.DragonLinks.Modules and _G.DragonLinks.Modules[Name]
    if not Link then return nil end
    local Success, Module = pcall(function()
        return loadstring(game:HttpGet(Link))()
    end)
    if Success then
        _G.DragonLoadedFeatureModules[Name] = Module or true
        return Module
    end
    return nil
end

_G.DragonLoadFunctionModule = function(Name)
    if not Name or _G.DragonSelectedMode ~= "HUB" then return nil end
    if _G.DragonLoadedFunctionModules[Name] then
        return _G.DragonLoadedFunctionModules[Name]
    end
    ...
    if Success then
        _G.DragonLoadedFunctionModules[Name] = Module or true
        return Module
    end
    return nil
end
...
task.spawn(function()
    ...
    function FastAttack:Attack()
        local AttackDelay = math.clamp(tonumber(_G.DragonAttackDelay) or Config.AttackCooldown, 0.05, 0.5)
  ...
  for _, Name in ipairs(Names) do
    ...
    if DragonFruitPrices[Normalized] then
      ...
      for FruitName, Price in pairs(DragonFruitPrices) do
        if #FruitName >= 3 and (string.find(Normalized, FruitName, 1, true) or string.find(FruitName, Normalized, 1, true)) then
...
UpdStFruit = function()
  ...
  for _, Item in ipairs(Fruits) do
    ...
    if StoreFruit then
      replicated.Remotes.CommF_:InvokeServer("StoreFruit", StoreFruit.Parent:GetAttribute("OriginalName"), Item)
    end
  end
end
...
local DragonUILibrary = (function()
  ...
  function Library:CreateWindow(Config)
    ...
    pcall(function() if protectgui then protectgui(Gui) elseif syn and syn.protect_gui then syn.protect_gui(Gui) end end)
    ...
    local Content = New("Frame", {Parent = Main, Size = UDim2.new(1, -(Config.TabWidth or 180) - 24, 1, -60), Position = UDim2.fromOffset((Config.TabWidth or 180) + 16, 54), BackgroundTransparency = 1})
    ...
    function Window:AddTab(TabConfig)
      ...
      New("UIPadding", {Parent = Page, PaddingTop = UDim.new(0, 4), PaddingLeft = UDim.new(0, 4), PaddingRight = UDim.new(0, 8), PaddingBottom = UDim.new(0, 8)})
      local Tab = {Page = Page, Button = Button, ModuleKey = TabConfig.Module}
      local function Base(Text, Height)
        local Frame = New("Frame", {Parent = Page, Size = UDim2.new(1, 0, 0, Height or 42), BackgroundColor3 = Color3.fromRGB(48, 31, 43), BorderSizePixel = 0})
        Round(Frame, 7)
        local Label = New("TextLabel", {Parent = Frame, Size = UDim2.new(1, -16, 1, 0), Position = UDim2.fromOffset(10, 0), BackgroundTransparency = 1, Text = Text or "", TextColor3 = Color3.fromRGB(240, 225, 233), TextSize = 13, Font = Enum.Font.Gotham, TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left})
        return Frame, Label
      end
      ...
      function Tab:AddParagraph(Options)
        ...
        Label.Font = Enum.Font.GothamBold
        local Desc = New("TextLabel", {Parent = Frame, Size = UDim2.new(1, -16, 0, 25), Position = UDim2.fromOffset(10, 24), BackgroundTransparency = 1, Text = Options.Content or "", TextColor3 = Color3.fromRGB(185, 170, 180), TextSize = 12, Font = Enum.Font.Gotham, TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left})
        local Control = {}
        function Control:SetDesc(Value) Desc.Text = tostring(Value or "") end
        function Control:SetTitle(Value) Label.Text = tostring(Value or "") end
        return Control
      end
      function Tab:AddButton(Options)
        local Frame = Base("", 40)
        local ButtonControl = New("TextButton", {Parent = Frame, Size = UDim2.new(1, -10, 1, -8), Position = UDim2.fromOffset(5, 4), BackgroundColor3 = Color3.fromRGB(115, 40, 70), BorderSizePixel = 0, Text = Options.Title or "Button", TextColor3 = Color3.new(1, 1, 1), TextSize = 13, Font = Enum.Font.GothamBold})
        ...
        ButtonControl.MouseButton1Click:Connect(function() pcall(Options.Callback or function() end) end)
        return ButtonControl
      end
      function Tab:AddToggle(Id, Options)
        local Frame, Label = Base(Options.Title or Id, 42)
        local ButtonControl = New("TextButton", {Parent = Frame, Size = UDim2.fromOffset(54, 26), Position = UDim2.new(1, -62, 0.5, -13), BackgroundColor3 = Color3.fromRGB(75, 65, 72), BorderSizePixel = 0, Text = "OFF", TextColor3 = Color3.new(1, 1, 1), TextSize = 11, Font = Enum.Font.GothamBold})
        Round(ButtonControl, 13)
        Label.Size = UDim2.new(1, -78, 1, 0)
        local Control = {Value = Options.Default == true, Callback = Options.Callback}
        function Control:Render() ButtonControl.Text = self.Value and "ON" or "OFF" ButtonControl.BackgroundColor3 = self.Value and Color3.fromRGB(185, 55, 100) or Color3.fromRGB(75, 65, 72) end
        function Control:SetValue(Value) self.Value = Value == true local Name = Options.Title or Id local Module = _G.DragonLoadFunctionModule(Name) or (Tab.ModuleKey and _G.DragonLoadFeatureModule(Tab.ModuleKey)) self:Render() if Module and Module.Name == Name and Module.Apply then Module:Apply(self.Value, self.Callback) elseif Module and Module.Apply then Module:Apply(Name, self.Value, self.Callback) elseif self.Callback then task.spawn(self.Callback, self.Value) end end
        function Control:OnChanged(Callback) self.Callback = Callback local Name = Options.Title or Id _G.DragonHubCallbacks[Name] = Callback local Pending = _G.DragonPendingHubValues[Name] if Pending ~= nil then self.Value = Pending _G.DragonPendingHubValues[Name] = nil end local Module = self.Value and (_G.DragonLoadFunctionModule(Name) or (Tab.ModuleKey and _G.DragonLoadFeatureModule(Tab.ModuleKey))) if Module and Module.Name == Name and Module.Apply then Module:Apply(self.Value, Callback) elseif Module and Module.Apply then Module:Apply(Name, self.Value, Callback) else task.spawn(Callback, self.Value) end end
        ButtonControl.MouseButton1Click:Connect(function() Control:SetValue(not Control.Value) end)
        Control:Render()
        return Control
      end
      function Tab:AddDropdown(Id, Options)
        ...
        function Control:SetValue(Value) self.Value = Value ButtonControl.Text = tostring(Value or "Select") if self.Callback then task.spawn(self.Callback, Value) end end
        function Control:OnChanged(Callback) local Name = Options.Title or Id self.Callback = Callback _G.DragonHubCallbacks[Name] = Callback _G.DragonHubControlInfo[Name] = {Type = "Dropdown", Values = self.Values, Value = self.Value} local Pending = _G.DragonPendingHubValues[Name] if Pending ~= nil then self.Value = Pending _G.DragonPendingHubValues[Name] = nil end if self.Value ~= nil then task.spawn(Callback, self.Value) end end
        ButtonControl.MouseButton1Click:Connect(function() if #Control.Values > 0 then Control.Index = Control.Index % #Control.Values + 1 Control:SetValue(Control.Values[Control.Index]) end end)
        return Control
      end
      function Tab:AddSlider(Id, Options)
        ...
        function Control:OnChanged(Callback) local Name = Options.Title or Id self.Callback = Callback _G.DragonHubCallbacks[Name] = Callback _G.DragonHubControlInfo[Name] = {Type = "Slider", Min = Min, Max = Max, Value = self.Value} local Pending = _G.DragonPendingHubValues[Name] if Pending ~= nil then self.Value = math.clamp(tonumber(Pending) or self.Value, Min, Max) _G.DragonPendingHubValues[Name] = nil self:Render() end task.spawn(Callback, self.Value) end
      ...
      function Tab:AddInput(Id, Options)
        ...
        local Input = New("TextBox", {Parent = Frame, Size = UDim2.new(0.53, 0, 0, 28), Position = UDim2.new(0.45, 0, 0.5, -14), BackgroundColor3 = Color3.fromRGB(75, 45, 63), BorderSizePixel = 0, Text = tostring(Options.Default or ""), PlaceholderText = Options.Placeholder or "", TextColor3 = Color3.new(1, 1, 1), TextSize = 12, ClearTextOnFocus = false})
        Round(Input, 6)
        Input.FocusLost:Connect(function() pcall(Options.Callback or function() end, Input.Text) end)
        return Input
      end
      Button.MouseButton1Click:Connect(function() Window:SelectTab(Index) end)
    ...
    function Window:SelectTab(Index)
      for Number, Tab in ipairs(self.Tabs) do
        ...
        Tab.Button.BackgroundColor3 = Number == Index and Color3.fromRGB(145, 45, 85) or Color3.fromRGB(55, 30, 45)
      end
      self.Current = Index
    end
    local Key = Config.MinimizeKey or Enum.KeyCode.End
    game:GetService("UserInputService").InputBegan:Connect(function(Input, Processed) if not Processed and Input.KeyCode == Key then Gui.Enabled = not Gui.Enabled end end)
    return Window
  end
  return Library
end)()
...
if not pcall(function() ScreenGui.Parent = CoreGui end) then ScreenGui.Parent = plr.PlayerGui end
...
function DragonUILibrary:CreateWindow()
  ...
  function Window:AddTab(TabConfig)
    ...
    function Tab:AddParagraph(Options)
      local Control = {Title = Options and Options.Title or "", Content = Options and Options.Content or ""}
      function Control:SetDesc(Value) self.Content = tostring(Value or "") end
      function Control:SetTitle(Value) self.Title = tostring(Value or "") end
      return Control
    end
    function Tab:AddButton(Options)
    ...
    function Tab:AddSlider(Id, Options)
      ...
      function Control:OnChanged(Callback)
        ...
        _G.DragonHubControlInfo[Name] = {Type = "Slider", Min = Options and Options.Min, Max = Options and Options.Max, Value = self.Value}
...
local Window = DragonUILibrary:CreateWindow({Title = "DragonHUB | HUB | " .. tostring(_G.DragonDetectedWorld),TabWidth = 145,Size = UDim2.fromOffset(530, 360),MinimizeKey = Enum.KeyCode.End})
  ...
  while wait(Sec) do
	if _G.Level then
	  pcall(function()
	    ...
	    if not QuestData[1] or not QuestData[2] or not QuestData[3] or not QuestData[4] or not QuestData[5] or not QuestData[6] then
	    ...
	    elseif QuestGui.Visible == true then
	      ...
	      if workspace.Enemies:FindFirstChild(QuestData[1]) then
		    ...
		    _tp(QuestData[4])
		    if replicated:FindFirstChild(QuestData[1]) then _tp(replicated:FindFirstChild(QuestData[1]).HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))end
		  end
		end
	  end)
	end
...
Sel_NFruit = Tabs.Fruit:AddDropdown("Sel_NFruit", {Title = "Select Fruit Stock", Values = DragonBasicFruitNames, Multi = false, Default = 1})
Sel_NFruit:OnChanged(function(Value)
  _G.SelectFruit = Value
end)

Tabs.Fruit:AddButton({Title = "Refresh Fruit Stock", Description = "Updates the fruits currently available", Callback = function()
...
Tabs.Fruit:AddButton({Title = "Buy Basic Stock", Description = "Buys the selected fruit if it is still in stock", Callback = function()
  PurchaseDragonFruit(_G.SelectFruit, false)
end})

Sel_MFruit = Tabs.Fruit:AddDropdown("Sel_MFruit", {Title = "Select Mirage Fruit", Values = DragonMirageFruitNames, Multi = false, Default = 1})
Sel_MFruit:OnChanged(function(Value)
  _G.SelectMirageFruit = Value
end)

Tabs.Fruit:AddButton({Title = "Buy Mirage Stock", Description = "Buys the selected Mirage stock fruit", Callback = function()
  PurchaseDragonFruit(_G.SelectMirageFruit, true)
end})

Sel_SniperFruit = Tabs.Fruit:AddDropdown("Sel_SniperFruit", {Title = "Select Fruit Sniper", Values = DragonFruitCatalogNames, Multi = false, Default = 1})
...
local AutoBuyFruitSniper = Tabs.Fruit:AddToggle("AutoBuyFruitSniper", {Title = "Auto Buy Selected Fruit", Description = "Buys the selected fruit when it appears in stock", Default = false})
    ...
    if _G.DragonSelectedMode == "AUTOMATIC" and _G.DragonAutoEnabled and (DragonStateDirty or os.clock() - DragonLastCheck >= 5) then
      ...
      pcall(function()
        ...
        elseif World3 then
          ...
          elseif MissingAny({"Saber", "Pole (1st Form)", "Shark Saw", "Wardens Sword", "Coat", "Pink Coat", "Usoap's Hat"}) then
            TravelToSea("Sea 1")
          elseif MissingAny({"Rengoku", "Death Step", "Sharkman Karate", "Koko", "Dragon Trident", "Midnight Blade", "Swan Glasses", "Longsword"}) then
            TravelToSea("Sea 2")
          ...
          elseif not Owns("Yama") then
            SetDragonObjective("Yama", "Getting Yama | Completing Elite Hunter progress", {Auto_Yama = true, FarmEliteHunt = true})
          elseif not Owns("Cursed Dual Katana") then
            SetDragonObjective("CDK", "Getting Cursed Dual Katana | Missing sword detected", {CDK = true, CDK_YM = true, CDK_TS = true})
          elseif not Owns("Skull Guitar") then
            SetDragonObjective("SkullGuitar", "Getting Skull Guitar | Missing weapon detected", {Auto_Soul_Guitar = true, AutoMatSoul = true})
          elseif not Owns("Godhuman") then
            SetDragonObjective("Godhuman", "Getting Godhuman | Missing style detected", {Auto_God_Human = true, AutoGodHuman = true})
          else
            local Goal = MissingDrop({
            ...
            if Goal then
              ...
              if ShopGoal then
                SetDragonObjective("Shop:" .. ShopGoal[1], "Buying " .. ShopGoal[1] .. " | Missing shop item", {Level = true})

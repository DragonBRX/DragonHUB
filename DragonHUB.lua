...
local Fluent = (function()
  ...
  function Library:CreateWindow(Config)
    ...
    local Old = Parent:FindFirstChild("DragonHUBInterface")
    if Old then Old:Destroy() end
    local Gui = New("ScreenGui", {Name = "DragonHUBInterface", ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling})
    ...
    function Window:AddTab(TabConfig)
      ...
      local Tab = {Page = Page, Button = Button}
      ...
      function Tab:AddToggle(Id, Options)
        ...
        function Control:SetValue(Value) self.Value = Value == true self:Render() if self.Callback then task.spawn(self.Callback, self.Value) end end
      ...
      function Tab:AddDropdown(Id, Options)
        ...
        function Control:OnChanged(Callback) self.Callback = Callback if self.Value ~= nil then task.spawn(Callback, self.Value) end end
...
local DragonGachaNextAttempt = 0

local function GetDragonGachaPrice()
  local LevelValue = plr.Data:FindFirstChild("Level")
  local CurrentLevel = LevelValue and LevelValue.Value or 1
  if CurrentLevel < 50 then return nil, CurrentLevel end
  local Price = 25000 + 150 * (CurrentLevel - 1)
...
local function GetDragonBeli()
  local BeliValue = plr.Data:FindFirstChild("Beli") or plr.Data:FindFirstChild("Money")
  return BeliValue and BeliValue.Value or 0
end

spawn(function()
  while task.wait(1) do
    if _G.Random_Auto and os.clock() >= DragonGachaNextAttempt then
      local Price, CurrentLevel = GetDragonGachaPrice()
      local CurrentBeli = GetDragonBeli()
      _G.DragonGachaPrice = Price or 32350
      _G.DragonGachaBeli = CurrentBeli
      if CurrentLevel < 50 then
        DragonGachaNextAttempt = os.clock() + 30
      else
        DragonGachaNextAttempt = os.clock() + 15
        local BeliBefore = CurrentBeli
        local BackpackBefore = #plr.Backpack:GetChildren()
        pcall(function()
          replicated.Remotes.CommF_:InvokeServer("Cousin", "Buy")
        end)
        task.wait(1)
        if GetDragonBeli() < BeliBefore or #plr.Backpack:GetChildren() > BackpackBefore then
          DragonGachaNextAttempt = os.clock() + 7200
          _G.StoreF = true
        end
      end
    end

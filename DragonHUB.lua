...
-- Carrega a interface via biblioteca externa
local NagaxLib = loadstring(game:HttpGet(
    "https://raw.githubusercontent.com/dragonbrxos/Nagax_HUB/refs/heads/main/UI/NagaxLib",
    true
))()

-- Detecta o mundo atual pelo PlaceId
local _currentWorld = 1
...
-- Registra o hub e define o teleporte inicial com base no mundo
NagaxLib:RegisterHub({
    hubName          = "Nagax_HUB",
    gameName         = "BloxFruits",
    version          = "5.0",
    selectiveButtons = false,
    teleport         = _currentWorld,
})

-- ── Criação das tabs ──────────────────────────────────────────
local Set        = NagaxLib:Tab("Farm")
local Main       = NagaxLib:Tab("Auto Farm")
local Farm       = NagaxLib:Tab("Quest/Items")
local Event      = NagaxLib:Tab("Sea")
local Stats      = NagaxLib:Tab("Stats")
local Tele       = NagaxLib:Tab("Teleport")
local Player     = NagaxLib:Tab("Player")
local Race       = NagaxLib:Tab("Race V4")
local Raid       = NagaxLib:Tab("Fruits/Raid")
local DemonFruit = NagaxLib:Tab("Visual")
local Esp        = NagaxLib:Tab("ESP")
local Shop       = NagaxLib:Tab("Shop")
local Misc       = NagaxLib:Tab("Misc")

-----------------------------------------------------------------------------------------------------------------------------Function
-- Protege toda a lógica de jogo: se qualquer erro ocorrer, a UI continua funcionando
pcall(function()
...
pcall(function() getgenv().A = require(game:GetService("ReplicatedStorage").CombatFramework.RigLib).wrapAttackAnimationAsync end)
pcall(function() getgenv().B = require(game.Players.LocalPlayer.PlayerScripts.CombatFramework.Particle).play end)
_G.setfflag = true
spawn(function()
    while wait() do
        if _G.setfflag then
...
-- Definições de posição iniciais (antes do loop que as usa)
PosX = 1
PosY = 35
PosZ = 40
Type = 1
...
if World1 then
    Main:Seperator("Farm Rapido Mar 1", { teleport = 1 })
...
Main:Seperator("Cake Farm", { teleport = 3 })
...
Main:Seperator("Bone Farm", { teleport = 3 })
...
Main:Seperator("Mastery de Arma", { teleport = _currentWorld })
...
-- ── Controles de ataque ──
-- Botoes para alternar o mar manualmente (util se detectado errado)
Set:Seperator("Selecionar Mar")
Set:Button("Mar 1  [Lv 1 - 700]", function()
    NagaxLib:SetTeleport(1)
end)
Set:Button("Mar 2  [Lv 700 - 1500]", function()
    NagaxLib:SetTeleport(2)
end)
Set:Button("Mar 3  [Lv 1500+]", function()
    NagaxLib:SetTeleport(3)
end)
Set:Seperator("Configuracoes de Farm")

Set:Toggle("Auto Haki", true, function(value)
    _G.AUTOHAKI = value
end)

-- Attack Version: troca entre Classic e Advanced (igual ao Select Weapon)
Set:Dropdown("Attack Version", {"Classic", "Advanced"}, function(value)
    _G.AttackVersion = value
end)
_G.AttackVersion = "Classic"

-- Auto Attack: liga/desliga o ataque do modo selecionado
Set:Toggle("Auto Attack", true, function(value)
    if _G.AttackVersion == "Classic" then
        _G.AutoClick = value
    else
        pcall(function()
...
-- Attack Mobs / Attack Players: controlam o FastAttack do Advanced
Set:Toggle("Attack Mobs", true, function(value)
    _G.AttackMobsEnabled = value
    pcall(function()
        if getgenv().rz_module and getgenv().rz_module.FastAttack then
            getgenv().rz_module.FastAttack.attackMobs = value
        end
    end)
end)

Set:Toggle("Attack Players", false, function(value)
    _G.AttackPlayersEnabled = value
    pcall(function()
        if getgenv().rz_module and getgenv().rz_module.FastAttack then
            getgenv().rz_module.FastAttack.attackPlayers = value
        end
    end)
end)
...
abc = true
    task.spawn(function()
        ...
        local c = nil
        pcall(function() c = require(game:GetService("ReplicatedStorage").CombatFramework.RigLib) end)
...
local old = nil
pcall(function() old = require(game:GetService("Players").LocalPlayer.PlayerScripts.CombatFramework) end)
local com = nil
pcall(function() if old and getupvalue then com = getupvalue(old, 2) end end)
...
pcall(function()
    if debug and debug.getupvalues then
        Combatfram1 = debug.getupvalues(require(ply.PlayerScripts.CombatFramework))
...
function Caclo()
    pcall(
        function()
            local a = game.Players.LocalPlayer
            local b = nil
            pcall(function() if getupvalues then b = getupvalues(require(a.PlayerScripts.CombatFramework))[2] end end)
...
-- Auto Haki foi movido para o topo do tab Farm (Set)
-- O loop de Auto Haki fica aqui
spawn(function()
    while wait(.1) do
        if _G.AUTOHAKI then 
            if not game.Players.LocalPlayer.Character:FindFirstChild("HasBuso") then
...
Farm:Seperator("Exclusivo Mar 2", { teleport = 2 })
...
Farm:Seperator("Exclusivo Mar 3", { teleport = 3 })
...
Farm:Seperator("Mar 2 / Acesso Mar 3", { teleport = 2 })
...
Farm:Seperator("Exclusivo Mar 1", { teleport = 1 })
...
Farm:Seperator("Evolucao de Raca - Mar 2", { teleport = 2 })
...
Farm:Seperator("Bartilo / Mar 3", { teleport = 3 })
...
Farm:Seperator("Elite Hunter", { teleport = 3 })
...
Farm:Seperator("Materiais Mar 1", { teleport = 1 })
...
Farm:Seperator("Materiais Mar 2", { teleport = 2 })
...
Farm:Seperator("Materiais Mar 3", { teleport = 3 })
...
Farm:Seperator("Espadas Mar 1", { teleport = 1 })
...
Farm:Seperator("Espadas Mar 2", { teleport = 2 })
...
Farm:Seperator("Espadas Mar 3", { teleport = 3 })
    ...
    spawn(function()
			while wait(.1) do
				if _G.FruitCheck then
					for i,v in pairs(game.Workspace:GetChildren()) do
						if string.find(v.Name, "Fruit") then
							pcall(function() require(game:GetService("ReplicatedStorage").Notification).new("Fruit Spawn Kìa"):Display() end);
                            wait()
                            setthreadcontext(5)
						end
					end
...
Raidslist = {}
pcall(function() RaidsModule = require(game.ReplicatedStorage.Raids) end)
...
-- ── Controle de altura do personagem durante o farm (PosY) ──
-- O personagem fica acima dos inimigos para atacar à distância sem receber dano
Misc:Slider("Farm Height (PosY)", 0, 150, 35, function(value)
    PosY = value
end)
...
Misc:Button("Kaitun Cap", function(value)
		local cac = nil
pcall(function() cac = require(game:GetService("Players").LocalPlayer.PlayerGui.Main.UIController.Inventory) end)
		...
		local total = 0
		pcall(function() if getupvalue then total = #getupvalue(cac.UpdateRender,4) end end)
		local rac = {}
		local allitem = {}
		local total2 = 0
...
Misc:Toggle("Remove Fog",RemoveFog,function(value)
    ...
    Misc:Toggle("Remove Damage", false, function()
